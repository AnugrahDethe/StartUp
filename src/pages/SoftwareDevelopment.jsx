...
const stats = [
  { target: 100, suffix: '+', label: 'Enterprise Systems' },
  { target: 30, suffix: '%', label: 'Cost Reduction' },
  { target: 10, suffix: 'x', label: 'ROI Average' },
...
const SoftwareDevelopment = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#030305] text-gray-200 overflow-x-hidden selection:bg-emerald-500/30">
                ...
                {process.map((step, idx) => {
                  ...
                  return (
                    <motion.div key={idx} initial={{ opacity: 0, y: 50, scale: 0.9 }} animate={inViewMethodology ? { opacity: 1, y: 0, scale: 1 } : {}} transition={{ delay: 0.5 + (idx * 0.2), type: "spring", stiffness: 100, damping: 12 }} className="flex flex-col items-center text-center group">
                      <motion.div whileHover={{ scale: 1.1, rotate: 5 }} className="w-24 h-24 rounded-full glass border-2 border-white/10 flex items-center justify-center bg-[#030305] mb-6 relative group-hover:border-emerald-500/50 transition-colors shadow-xl">
